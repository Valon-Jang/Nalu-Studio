...
class FastSpeakerApp:
    def __init__(self, root: tk.Tk, repo_root: Path) -> None:
        ...
        ttk.Button(controls, text="Same", command=lambda: self.retest(RetestOutcome.SAME)).pack(side="left", padx=2)
        ttk.Button(controls, text="Worse", command=lambda: self.retest(RetestOutcome.WORSE)).pack(side="left", padx=2)
